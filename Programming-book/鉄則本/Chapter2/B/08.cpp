...
int main() {
    int N;
    cin >> N;
    vector<vector<int>> points(1510, vector<int>(1510, 0));
    rep(i, 0, N) {
        int X, Y;
        cin >> X >> Y;
        X--;
    ...
    vector<vector<int>> cs2(1510 + 1, vector<int>(1510 + 1, 0));
    rep(i, 0, 1510) rep(j, 0, 1510) cs2[i + 1][j + 1] = cs2[i + 1][j] + cs2[i][j + 1] - cs2[i][j] + points[i][j];
    ...
    rep(i, 0, Q) {
        ...
        cout << cs2[c][d] - cs2[a - 1][d] - cs2[c][b - 1] + cs2[a - 1][b - 1] << endl;
    }
}