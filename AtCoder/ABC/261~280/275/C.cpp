...
int main() {
    vector<Pair_int> point;
    rep(i, 0, 9) {
        string S;
        cin >> S;
        rep(j, 0, 9) if (S[j] == '#') point.push_back(make_pair(i, j));
    }
    ...
    int ans = 0;
    rep(i, 0, size) rep(j, i + 1, size) rep(k, j + 1, size) rep(l, k + 1, size) {
        vector<int> x, y;
        x.push_back(point[i].first); y.push_back(point[i].second);
        x.push_back(point[j].first); y.push_back(point[j].second);
        x.push_back(point[k].first); y.push_back(point[k].second);
        x.push_back(point[l].first); y.push_back(point[l].second);
